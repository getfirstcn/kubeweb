/* Parsley dist/parsley.min.js build version 1.2.3 http://parsleyjs.org */ ! function(d) {
    var h = function(a) {
        this.messages = {
            ...
            max: "此值应低于或等于 %s。",
            range: "此值应为 %s 和 %s 之间。",
            minlength: "这个值是太短了。它应具有 %s 的字符或更多。",
            maxlength: "此值太长。它应具有 %s 的字符或更少。",
            rangelength: "该值长度无效。它应该是 %s 和 %s 字符之间。",
            mincheck: "你必须选择至少 %s 的选择。",
            maxcheck: "您必须选择 %s 的选择或更少。",
            rangecheck: "您必须 %s 和 %s 的选择之间进行选择。",
            equalto: "此值应是相同的。"
        };
        this.init(a)
    };
    h.prototype = {
        ...
        addValidator: function(a, b) {
            if ("undefined" === typeof b().validate) throw Error("验证 `" + a + "` 必须有一个validate方法。查看更多在这里: http://parsleyjs.org/documentation.html#javascript-general");
            "undefined" === typeof b().priority && (b = {
                    validate: b().validate,
                    priority: 32
                },
                window.console && window.console.warn && window.console.warn("验证 `" + a + "` 应该有一个优先事项。默认优先给予32"));
            this.validators[a] = b
        },
    ...
    p.prototype = {
        ...
        getName: function() {
            ...
            if ("undefined" === typeof this.$element.attr("name")) throw "A radio / 复选框输入必须有一个 parsley-group 属性或名称是 Parsley validated !";
